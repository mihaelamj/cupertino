...
The default behavior of `cupertino fetch --source swift-evolution` is `--only-accepted`. Only proposals in `Accepted`, `Implemented`, `Active review`, or `Awaiting implementation` status are downloaded. `--no-only-accepted` flips that off, including proposals in `Withdrawn`, `Rejected`, `Returned for revision`, and any other status.
...
- Only meaningful with `--source swift-evolution`. Ignored otherwise.
