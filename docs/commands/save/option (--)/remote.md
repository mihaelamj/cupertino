...
Builds the docs-bucket per-source databases (`apple-documentation.db` and its siblings) by streaming raw documentation pages from the public `mihaelamj/cupertino-docs` GitHub repository instead of reading them from a local `~/.cupertino/docs/` tree. Useful for instant first-time setup, no `cupertino fetch` step required.
...
### Build the docs index without crawling first
```bash
cupertino save --remote
```
...
- Doesn't apply to `--source samples` or `--source packages` builds, those still read local extracted archives.
