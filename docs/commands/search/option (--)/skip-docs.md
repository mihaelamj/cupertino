...
In fan-out mode (no `--source`), excludes apple-docs, apple-archive, hig, swift-evolution, swift-org, and swift-book from the search. Useful when `search.db` is locked (e.g. another process running `cupertino save --source apple-docs`) or when you only want package + sample results. ([#239](https://github.com/mihaelamj/cupertino/issues/239))
