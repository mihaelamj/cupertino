...
```bash
cupertino search-symbols --query <substring> --kind <kind> --format <text|json|markdown|md>
```
...
```bash
cupertino search-symbols --query Task --kind struct --format json | jq '.results[0]'
```
