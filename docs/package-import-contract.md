...
| `HIGSource` | Foundation, ASTIndexer, CoreProtocols, LoggingModels, SearchModels, SearchStrategyHelpers, SharedConstants | ✅ Foundation, ASTIndexer, CoreProtocols, LoggingModels, SearchModels, SearchStrategyHelpers, SharedConstants (#1010 / #1007: per-source target absorbing the prior `HIGStrategy` target plus the `HIGIndexer` concrete lifted from `SearchSQLite`. Exposes `HIGSource: Search.SourceProvider` carrying `definition`, `fetchInfo`, `makeStrategy`, `makeIndexer`. `ASTIndexer` is the direct import in `Search.HIGIndexer.swift` for `Search.ExtractedContent` namespace resolution. Second per-source target of the source-unification epic; phase 1A landed AppleDocsSource via #1008.) |
