...
| `Enrichment` | Foundation, EnrichmentModels, SearchModels, SampleIndexModels, SharedConstants (TARGET — see Current state) | ⚠️ Foundation, EnrichmentModels, SampleIndex, SampleIndexModels, Search, SearchModels, SharedConstants. 6 sibling passes import `Search` + `SampleIndex` concretes directly (write-side coupling). Not yet added to `STRICT_PRODUCERS`; #906 (child of epic #893) splits each pass into its own SPM target consuming `EnrichmentModels.EnrichmentPass` against protocol-fronted Search / SampleIndex writers |
...
**30 producers match the target regime + are opted into `scripts/check-target-foundation-only.sh`'s `STRICT_PRODUCERS`** (12 `*Models` seams including `EnrichmentModels`, 17 feature producers + `AppleConstraintsKit`). `Enrichment` is documented above but not opted in yet, awaiting the per-pass split in #906.
