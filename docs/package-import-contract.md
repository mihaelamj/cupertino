...
| `HIGSource` | Foundation, CoreProtocols, LoggingModels, SearchModels, SearchStrategyHelpers, SharedConstants | ✅ Foundation, CoreProtocols, LoggingModels, SearchModels, SearchStrategyHelpers, SharedConstants (#1010 / #1007: per-source target absorbing the prior `HIGStrategy` target plus the `HIGIndexer` concrete lifted from `SearchSQLite`. Exposes `HIGSource: Search.SourceProvider` carrying `definition`, `fetchInfo`, `makeStrategy`, `makeIndexer`. Diverges from the AppleDocsSource row: no `ASTIndexer` dep, because `Search.HIGIndexer.extractCode` returns `.empty` (HIG is pure design guidance, no code extraction) and references no `ASTIndexer.*` symbols. Second per-source target of the source-unification epic; phase 1A landed AppleDocsSource via #1008.) |
| `SampleCodeSource` | Foundation, ASTIndexer, CoreProtocols, LoggingModels, SearchModels, SearchStrategyHelpers, SharedConstants | ✅ Foundation, ASTIndexer, CoreProtocols, LoggingModels, SearchModels, SearchStrategyHelpers, SharedConstants (#1012 / #1007: per-source target absorbing the prior `SampleCodeStrategy` target plus the `SampleCodeIndexer` concrete lifted from `SearchSQLite`. Exposes `SampleCodeSource: Search.SourceProvider` carrying `definition`, `fetchInfo`, `makeStrategy`, `makeIndexer`. ASTIndexer dep is load-bearing here (unlike HIGSource): `Search.SampleCodeIndexer.extractCode` runs `ASTIndexer.Extractor` over full Swift files. Third per-source target of the source-unification epic; first to introduce a per-source runtime dep `env.sampleCatalogProvider` on `Search.IndexEnvironment` (optional field, AppleDocs / HIG ignore it; SampleCodeSource preconditions non-nil at `makeStrategy(env:)` time).) |
| `SwiftEvolutionSource` | Foundation, ASTIndexer, CoreProtocols, LoggingModels, SearchModels, SearchStrategyHelpers, SharedConstants | ✅ Foundation, ASTIndexer, CoreProtocols, LoggingModels, SearchModels, SearchStrategyHelpers, SharedConstants (#1017 / #1007: per-source target absorbing the prior `SwiftEvolutionStrategy` target plus the `SwiftEvolutionIndexer` concrete lifted from `SearchSQLite`. ASTIndexer dep is load-bearing: `Search.SwiftEvolutionIndexer.extractCode` pulls Swift code blocks out of proposal markdown via the private `extractAllCodeBlocks` helper and runs `ASTIndexer.Extractor` over them. Fifth per-source target of the source-unification epic; declares `destinationDB = .search` per the post-#1014 protocol requirement.) |
| `SwiftOrgSource` | Foundation, CoreProtocols, LoggingModels, SearchModels, SearchStrategyHelpers, SharedConstants | ✅ Foundation, CoreProtocols, LoggingModels, SearchModels, SearchStrategyHelpers, SharedConstants (#1019 / #1007: per-source target absorbing the prior `SwiftOrgStrategy` target plus the `SwiftOrgIndexer` concrete lifted from `SearchSQLite`. **No ASTIndexer dep** (per the per-source-dep-set rule from HIGSource): `Search.SwiftOrgIndexer` uses the default `extractCode` returning `.empty`. Sixth per-source target; declares `destinationDB = .search`.) |
| `AppleArchiveSource` | Foundation, ASTIndexer, CoreProtocols, LoggingModels, SearchModels, SearchStrategyHelpers, SharedConstants | ✅ Foundation, ASTIndexer, CoreProtocols, LoggingModels, SearchModels, SearchStrategyHelpers, SharedConstants (#1014 / #1007: per-source target absorbing the prior `AppleArchiveStrategy` target plus the `AppleArchiveIndexer` concrete lifted from `SearchSQLite`. Exposes `AppleArchiveSource: Search.SourceProvider` carrying `definition`, `fetchInfo`, `destinationDB`, `makeStrategy`, `makeIndexer`. ASTIndexer dep is load-bearing: `Search.AppleArchiveIndexer.extractCode` runs `ASTIndexer.Extractor` conditionally on Swift-shaped content. Fourth per-source target of the source-unification epic; PR #1014 also lands the protocol-level `destinationDB: Shared.Models.DatabaseDescriptor` requirement (no implicit search.db routing; every conformer declares; #1015 absorbed into #1014's scope). All 4 migrated per-source targets declare `destinationDB = .search`.) |
