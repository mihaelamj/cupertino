...
| Source-specific strategies | per-source SPM packages: `Packages/Sources/{HIGSource,AppleDocsSource,AppleArchiveStrategy,SampleCodeStrategy,SwiftEvolutionStrategy,SwiftOrgStrategy}/Search.Strategies.*.swift` (AppleDocsSource renamed from AppleDocsStrategy in #1008 as the first per-source SPM target) |
