...
extension Search {
    ...
    public actor PackageQuery {
        ...
        public init(dbPath: URL) async throws {
            self.dbPath = dbPath
            // #1194: open through the one shared read-only path every reader
            // uses (`SQLiteSupport.openReadOnly`). A plain `SQLITE_OPEN_READONLY`
            // open needs no `-shm` on a rollback-mode shipped DB (#1192), which
            // is how packages.db now ships; the handle is incapable of writing,
            // so this query path cannot mutate the corpus.
            do {
        ...
        /// opened this reader read-write (dropping the read-only open) would
        /// make a write SUCCEED (`SQLITE_OK`) here instead of `SQLITE_READONLY`.
