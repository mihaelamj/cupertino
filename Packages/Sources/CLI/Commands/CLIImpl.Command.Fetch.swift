...
extension CLIImpl.Command {
    ...
    struct Fetch: AsyncParsableCommand {
        ...
        private func runPackageAnnotationStage(outputURL: URL) async throws {
            ...
            let owners = (try? FileSystem.contentsOfDirectory(at: outputURL, includingPropertiesForKeys: nil))?
            ...
            for ownerURL in owners.sorted(by: { $0.lastPathComponent < $1.lastPathComponent }) {
                let repos = (try? FileSystem.contentsOfDirectory(at: ownerURL, includingPropertiesForKeys: nil))?
                    .filter { (try? $0.resourceValues(forKeys: [.isDirectoryKey]).isDirectory) == true }
                    .filter { !$0.lastPathComponent.hasPrefix(".") }
                    ?? []
