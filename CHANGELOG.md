...
_Binary release on top of v1.0.1. `databaseVersion` stays at `1.0.0`: `cupertino setup` from a v1.0.2 binary downloads the same `cupertino-databases-v1.0.0.zip` bundle as earlier 1.0.x releases. v1.0.2 is the first release to land both halves of the v1.0.x crawler-correctness work — the post-redirect URL canonicalization that #277 surfaced and the HTML link augmentation that #203 has been waiting for. Neither change rewrites the bundled DB; the next refreshed bundle would ship under a separate corpus tag (current unreleased corpus state is tagged at `mihaelamj/cupertino-docs@v1.0.2` as a snapshot, no GitHub Release attached yet)._
