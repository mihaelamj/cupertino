...
- **Foundation-only producer epic (#536), phases 0 + 1 + 2a shipped 2026-05-15:**
  - **Phase 0 (#537):** `scripts/check-target-foundation-only.sh` — per-target allow-list guard with opt-in `STRICT_PRODUCERS` array (empty during phases 0-2; phase 3 will opt producers in one by one). `docs/package-import-contract.md` rewritten with the post-#536 target regime + Apple-pattern corroboration (SwiftNIO `NIOCore`, swift-log, everliv-monorepo, GoF Strategy p. 315 / Factory Method p. 107, Point-Free swift-dependencies). Global rule at `mihaela-agents/Rules/swift/per-package-import-contract.md` updated.
  - **Phase 1 — Shared-layer consolidation:** the four legacy `Shared*` sub-targets absorbed into `SharedConstants`.
    - **1a (#538):** `SharedCore` (`Shared.Core.ToolError`, `Shared.Core` anchor, `CupertinoShared.swift`) → `SharedConstants`. 3 files moved, 179 import-line cleanups, `@_exported import SharedCore` dropped from `Core/CupertinoCore.swift`. Bonus fix: `LoggingTests` target had `dependencies: ["LoggingModels", "LoggingModels", "TestSupport"]` (duplicate + missing `Logging`) — replaced with `["Logging", "LoggingModels", "TestSupport"]`.
    ...
    - **Net result:** Shared layer collapses from 5 targets (`SharedConstants`, `SharedCore`, `SharedUtils`, `SharedModels`, `SharedConfiguration`) to 1 (`SharedConstants`). Every producer now imports a single Shared target instead of up to five.
  - **Phase 2a (#542):** content audit of `CoreProtocols` for foundation-only purity. Two leaks (`Core.Protocols.GitHubCanonicalizer` public actor with URLSession + FileManager cache; `Core.Protocols.ExclusionList` enum with `Data(contentsOf:)` loader) moved out of the protocol-seam target into `CorePackageIndexing` (their actual consumer). Namespace rewrites `extension Core.Protocols` → `extension Core.PackageIndexing`. Three smoke tests followed the impls into `CorePackageIndexingTests/CorePackageIndexing.MovedFromCoreProtocols.swift`. CoreProtocols is now genuinely foundation-only.
  - **Audit result post-2a:** every producer target's actual `^import` lines already match the foundation-only target regime (external + foundation tier + `*Models` seams; no other producer's writer; no `Logging` writer; no `SharedCore`/`SharedUtils`/`SharedModels`/`SharedConfiguration`). Phase 3 will be mechanical opt-in via `STRICT_PRODUCERS`.
