...
- **#593 — samples indexer stops corrupting every file path with a leading `/private` prefix.** Caught during main's post-#589/#590 retest sweep. Every row in `samples.db.files.path` was getting a `/private` prefix concatenated without separator (`/privateShared/foo.swift` instead of `Shared/foo.swift`), making `cupertino read-sample-file '<id>' '<path>'` return "File not found" for the (correctly spelled) path the user expects. Root cause was in `Sample.Index.Builder.discoverFiles(...)`:
  ...
  **Correction (added post-merge, 2026-05-15):** an earlier draft of this entry claimed the bug was "not in the shipped bundle". Main's full-coverage sweep against the shipped `samples.db` from `cupertino setup` showed **18,911 of 18,928 rows (99.9%) carry the `/private` prefix corruption** — the shipped bundle IS affected. Every user who ran `cupertino setup` for v1.0.2 has the corrupted paths. Fix path is a bundle re-publish with a re-indexed `samples.db` (no code change needed beyond what's in this PR). Users with a locally-rebuilt samples.db can re-run `cupertino save --samples --clear`; users on the shipped bundle wait for the next bundle re-publish. Full 1732-test suite green.
